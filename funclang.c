...
int saveItemDetails(const struct ItemDetails *arr, size_t nmemb, int fd) {
  ...
  fwrite(&nmemb, sizeof(size_t), 1,
         fptr); // header specified in the project-spec

  // Assuming that the items are already sanitized, loop each ItemDetails in the
  // array and write each one.
  for (size_t i = 0; i < nmemb; i++) {
...
/**
 * checks whether an ItemDetails struct is valid – it is valid iff all of its
fields are valid (as described in the documentation for the struct and elsewhere
in this project specification). The name and desc fields must be valid name and
multi-word fields, respectively; they also must not be empty strings. This
function returns 1 if the struct is valid, and 0 if not.
*/
...
/**
 * checks whether a Character struct is valid – it is valid iff all of its
fields are valid (as described in the documentation for the struct and elsewhere
in this project specification). The following are all necessary preconditions
for validity of the struct: • the profession field must be a valid name field,
and must not be the empty string; • the name field must be a valid multi-word
field, and must not be the empty string; • the total number of items carried
(that is: the sum of the quantity fields of the ItemCarried structs that form
part of the inventory) must not exceed MAX_ITEMS; and • inventorySize must be
less than or equal to MAX_ITEMS. This function returns 1 if the struct is valid,
and 0 if not.
*/
...
int saveCharacters(struct Character *arr, size_t nmemb, int fd) { return 0; }
...
int main() {
  ...
  struct ItemDetails itemArr[] = {
      {.itemID = 16602759796824695000UL,
       .name = "telescope",
       .desc = "brass with wooden tripod, 25x30x60 in."}};
  ...
  //     printf("%lu \t %s\t %s \n", itemsArr[i].itemID, itemsArr[i].name,
  //     itemsArr[i].desc);
  // }
